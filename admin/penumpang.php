<?php
	...
	$jumlahdatapenumpang	= count(tampilpenumpang("SELECT * FROM penumpang"));
	...
	$penumpang = tampilpenumpang("SELECT * FROM penumpang ORDER BY id_penumpang DESC LIMIT $indexhalamanpenumpang,$jumlahpenumpangtampil ");
	

	if ( isset($_POST['cari'] ) ) 
	{
		$penumpang = caripenumpang($_POST['keyword']);	
	}
	...
	<div class="row no-gutters mt-5">
		...
		<div class="col-md-10 p-5 pt-2">
			<h3 class="text-primary">
				<a href="penumpang.php"><i class="fas fa-users mr-2" data-toggle="tooltip" title="Daftar Penumpang"></i> Daftar Penumpang</a>
			</h3><hr>
					...
					<ul class="navbar-nav mr-auto">
						<li>
							<form class="form-inline my-2" action="" method="post">
								<input class="form-control mr-sm-2" type="search" name="keyword" size="50" autofocus autocomplete="off" placeholder="Cari Penumpang Duta Enim Wisata Travel">
								<button class="btn btn-primary btn-lg" type="submit" name="cari"><i class="fa fa-search"></i></button>
							</form>
						</li>
					</ul>
					<a href="tambahpenumpang.php" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Penumpang</a>
				</div>
			</nav>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr>
						...
						<td><?php echo $pnpg["tanggalkeberangkatan_penumpang"]; ?></td>
						<td><?php echo $pnpg["nohp_penumpang"]; ?></td>
						<td><?php echo $pnpg["jumlah_penumpang"]; ?></td>
						<td><?php echo $pnpg["alamat_penumpang"]; ?></td>
						<td> 
							<a href="ubahpenumpang.php?id_penumpang=<?= $pnpg['id_penumpang'];?>"><i class="fa fa-edit fa-2x mr-2" data-toggle="tooltip" title="Ubah"></i></a>
							<a href="hapuspenumpang.php?id_penumpang=<?= $pnpg["id_penumpang"];?>" onclick = "return confirm('Apakah anda yakin ?');"><i class="fa fa-trash-alt fa-2x text-danger" data-toggle="tooltip" title="Hapus"></i></a>
						</td>
					</tr>
				</tbody>
				<?php $i++; endforeach; ?>
			...
			<nav>
				<ul class="pagination justify-content-center">
					...
					<?php if ($halamanaktif < $jumlahhalamanpenumpang): ?>
						<li class="page-item">
							<a class="page-link" href="?halaman=<?= $jumlahhalamanpenumpang;?>">Halaman Terakhir &raquo;</a>
						</li>
					<?php endif ?>
